...
% generate blank files
    ...
    AMP1=zeros(length(fp),1);
    AMP2=zeros(length(fp),1);
...
goodloc=find(toa>0);
for m=1:length(goodloc);
    n=goodloc(m);
    fp(n)/1000
    invdt=1./(tob(n)-toa(n));
    V(1,n)=(zrec2-zrec1).*invdt;
